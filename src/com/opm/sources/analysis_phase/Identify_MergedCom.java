...
public class Identify_MergedCom {
    ...
    private static void indentify() throws Exception {
        ...
        for (int a = 0; a < FILES.length; a++) {
            int sheet_index = 0;
            int total_sheets = File_Details.getWorksheets(path_merged + FILES[a]);
            //System.out.println("Reading Collection Excel....!");
            while (sheet_index < total_sheets) {

                String project = File_Details.setProjectName(path_merged + FILES[a], sheet_index, "B2");
                String pChange = File_Details.setProjectName(path_merged + FILES[a], sheet_index, "G2");
                List<String> nList = ReadExcelFile_1Column.readColumnAsString(path_merged + FILES[a], sheet_index, 6, 2);
                //List<String> dList = Pick_GeneralNext.pick(path_merged + FILES[a], sheet_index, 6, 2);
        ...
        List<String> fork_namesList = ReadExcelFile_1Column.readColumnAsString(path + file1, 0, 1, 1);
        ...
        int token_index = 0;
        for (int i = 0; i < fork_namesList.size(); i++) {

            if (token_index == (Constants.getToken().length)) {/// the the index for the tokens array...
                token_index = 0; //// go back to the first index......
            }
            String url = "https://api.github.com/repos/" + fork_namesList.get(i) + "?access_token=" + Constants.getToken()[token_index++];
        ...
        for (int a = 32; a < pList.size(); a++) {
            ...
            if (proj_list.contains(pList.get(a))) {
                ...
                for (int j = 0; j < fork_namesList.size(); j++) {
                    if (pList.get(a).equals(projList.get(j))) {
                        nameL.add(fork_namesList.get(j));
                ...
                List<List<String>> allList_1 = Commits.count(pList.get(a), "mlp", min_date, Constants.cons.TODAY_DATE, Constants.getToken(), token_index);
                ...
                for (int j = 0; j < nameL.size(); j++) {
                    ...
                    List<List<String>> allList = Commits.count(nameL.get(j), "fp", createdL.get(j), Constants.cons.TODAY_DATE, Constants.getToken(), token_index);
                    lists.add(allList);
                }
                ...
                for (int b = 0; b < nameL.size(); b++) {
                    ...
                    for (int i = 0; i < mergedList.size(); i++) {
                        ...
                        String commits_details = Shaa_Details.details1(nameL.get(b), mergedList.get(i), Constants.getToken(), token_index);
                        //System.out.println(shaa+"\t"+commits_details);
                        if (commits_details != null) {
                            token_index = Integer.parseInt(commits_details.split("/")[commits_details.split("/").length - 1]);
